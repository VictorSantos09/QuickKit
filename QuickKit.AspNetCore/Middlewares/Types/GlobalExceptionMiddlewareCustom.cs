...
public class GlobalExceptionMiddlewareCustom<TException> where TException : Exception
{
    private readonly GlobalExceptionMiddlewareConfigurationRequest<TException> _config;

    /// <summary>
    /// Initializes a new instance of the <see cref="GlobalExceptionMiddlewareCustom{TException}"/> class.
    /// </summary>
    /// <param name="config">The configuration for the middleware.</param>
    public GlobalExceptionMiddlewareCustom(GlobalExceptionMiddlewareConfigurationRequest<TException> config)
    {
        _config = config;
    }
